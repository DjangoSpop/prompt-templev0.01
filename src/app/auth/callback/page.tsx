...
function AuthCallbackContent() {
  ...
  const { handleAuthCallback, isAuthenticating, authError } = useSocialAuth({
  ...
  useEffect(() => {
    ...
    if (error) {
      const errorDescription =
        searchParams.get('error_description') || 'Authentication was cancelled';
      setStatus('error');
              ...
              {status === 'processing' && (
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-temple-stone">
                    Verifying credentials with the ancient guardians...
                  </p>
                </div>
              )}
              ...
              {status === 'error' && (
                <div className="space-y-2">
                  <p className="text-sm text-red-700">
                    {authError || 'Please try again or contact support if the issue persists.'}
                  </p>
                  <p className="text-xs text-temple-stone">
...
function AuthCallbackFallback() {
  return (
    <div className="min-h-screen bg-gradient-to-br from-desert-sand/30 via-background to-nile-teal/20 flex items-center justify-center px-4 py-8 relative overflow-hidden">
      <div className="absolute inset-0">
        <div className="absolute top-1/4 left-1/4 w-72 h-72 bg-pharaoh-gold/20 rounded-full mix-blend-multiply filter blur-xl opacity-20 animate-pulse"></div>
        <div className="absolute top-1/3 right-1/4 w-72 h-72 bg-nile-teal/20 rounded-full mix-blend-multiply filter blur-xl opacity-20 animate-pulse animation-delay-2000"></div>
      </div>
      ...
      <div className="w-full max-w-md space-y-8 relative z-10">
        ...
        <Card className="bg-gradient-to-br from-pharaoh-gold/10 to-nile-teal/10 backdrop-blur-lg border border-pharaoh-gold/20 shadow-pyramid rounded-temple">
          <CardContent className="p-8">
            <div className="flex flex-col items-center space-y-4 text-center">
              <Loader2 className="h-8 w-8 text-pharaoh-gold animate-spin" />
              <h2 className="text-xl font-bold text-foreground">Preparing authentication...</h2>
              <p className="text-sm text-temple-stone">
                Hold tight while we gather your sacred credentials.
              </p>
            </div>
          </CardContent>
        </Card>
