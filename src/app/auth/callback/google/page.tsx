...
function GoogleCallbackContent() {
  ...
  const { handleAuthCallback } = useSocialAuth({
    ...
    onAuthSuccess: (response) => {
      ...
      setTimeout(() => {
        ...
        sessionStorage.removeItem('google_callback_processing');
  ...
  useEffect(() => {
    ...
    const processingKey = 'google_callback_processing';
    ...
    console.log('🔍 Google Callback URL params:', { code: code?.substring(0, 20), state: state?.substring(0, 20), error });
    console.log('🔍 Full URL:', window.location.href);
    
    // If no parameters at all, something is very wrong - redirect immediately
    if (!code && !state && !error) {
    ...
    if (sessionStorage.getItem(processingKey)) {
      console.log('⏭️ Google callback already processing, skipping...');
      return;
    }
    ...
    if (error) {
      const errorDescription =
        searchParams.get('error_description') || 'Google authentication was cancelled';
    ...
    if (code && state) {
      ...
      console.log('📦 LocalStorage state check:', {
        ...
        hasOAuthProvider: !!localStorage.getItem('oauth_provider'),
        hasOAuthTimestamp: !!localStorage.getItem('oauth_timestamp'),
        statePreview: localStorage.getItem('oauth_state')?.substring(0, 30) + '...'
      });
      // Handle the OAuth callback with 'google' provider
      handleAuthCallback(code, state, 'google');
    } else {
      console.error('❌ Missing code or state parameter');
      setStatus('error');
      setMessage('Invalid callback parameters from Google');
      sessionStorage.removeItem(processingKey);
        ...
        <div className="text-center">
          <div className="flex items-center justify-center space-x-3 mb-6">
            ...
            <div>
              ...
              <p className="text-temple-stone text-sm">Google Sign-In</p>
            </div>
          </div>
        </div>
