 ...
 *
 * Automate LinkedIn publishing from a Notion content calendar
 */
...
export default function Module8Page() {
  const module = getModuleById('module-8');
  const [currentLessonIndex, setCurrentLessonIndex] = useState(0);
  const [showQuiz, setShowQuiz] = useState(false);
  const unlockedModules = useAcademyStore((state) => state.unlockedModules);
  const progress = useAcademyStore(selectModuleProgress('module-8'));

  // Check if user has unlocked this module
  const isUnlocked = unlockedModules.includes('module-8');
          ...
          {showQuiz ? (
            <div>
              <h2 className="text-3xl font-bold text-amber-400 mb-6">
                The LinkedIn Automation Trial
              </h2>
              <QuizEngine quiz={module.quiz} moduleId="module-8" />
            </div>
          ) : (
            <div>
              <div className="mb-6">
