 ...
 *
 * Process YouTube playlists with AI summarization and RAG Q&A
 */
...
export default function Module9Page() {
  const module = getModuleById('module-9');
  const [currentLessonIndex, setCurrentLessonIndex] = useState(0);
  const [showQuiz, setShowQuiz] = useState(false);
  const unlockedModules = useAcademyStore((state) => state.unlockedModules);
  const progress = useAcademyStore(selectModuleProgress('module-9'));

  const isUnlocked = unlockedModules.includes('module-9');
          ...
          {showQuiz ? (
            <div>
              <h2 className="text-3xl font-bold text-amber-400 mb-6">
                The YouTube AI Processing Trial
              </h2>
              <QuizEngine quiz={module.quiz} moduleId="module-9" />
            </div>
          ) : (
            <div>
              <div className="mb-6">
