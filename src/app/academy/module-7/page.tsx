 ...
 *
 * Master MCP, AI agents, and context engineering
 */
...
export default function Module7Page() {
  const module = getModuleById('module-7');
  const [currentLessonIndex, setCurrentLessonIndex] = useState(0);
  const [showQuiz, setShowQuiz] = useState(false);
  const unlockedModules = useAcademyStore((state) => state.unlockedModules);
  const progress = useAcademyStore(selectModuleProgress('module-7'));

  // Check if user has unlocked this module
  const isUnlocked = unlockedModules.includes('module-7');
        ...
        <main className="flex-1 overflow-y-auto p-6 md:p-10">
          {showQuiz ? (
            <div>
              ...
              <QuizEngine quiz={module.quiz} moduleId="module-7" />
            </div>
          ) : (
            <div>
              <div className="mb-6">
