 ...
 *
 * Automate lead generation with Google Maps scraping and email extraction
 */
...
export default function Module10Page() {
  const module = getModuleById('module-10');
  const [currentLessonIndex, setCurrentLessonIndex] = useState(0);
  const [showQuiz, setShowQuiz] = useState(false);
  const unlockedModules = useAcademyStore((state) => state.unlockedModules);
  const progress = useAcademyStore(selectModuleProgress('module-10'));

  const isUnlocked = unlockedModules.includes('module-10');
          ...
          {showQuiz ? (
            <div>
              <h2 className="text-3xl font-bold text-amber-400 mb-6">
                The Lead Generation Trial
              </h2>
              <QuizEngine quiz={module.quiz} moduleId="module-10" />
            </div>
          ) : (
            <div>
              <div className="mb-6">
