 ...
 *
 * Multi-platform content publishing with AI orchestration
 */
...
export default function Module12Page() {
  const module = getModuleById('module-12');
  const [currentLessonIndex, setCurrentLessonIndex] = useState(0);
  const [showQuiz, setShowQuiz] = useState(false);
  const unlockedModules = useAcademyStore((state) => state.unlockedModules);
  const progress = useAcademyStore(selectModuleProgress('module-12'));

  const isUnlocked = unlockedModules.includes('module-12');
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-obsidian-900">
      ...
      <div className="flex flex-1 min-h-0">
        <ModuleSidebar moduleId="module-12" currentLessonIndex={currentLessonIndex} showQuiz={showQuiz} onLessonClick={(index) => { setShowQuiz(false); setCurrentLessonIndex(index); }} onQuizClick={() => setShowQuiz(true)} />
        <main className="flex-1 overflow-y-auto p-6 md:p-10">
          {showQuiz ? (
            <div>
              <h2 className="text-3xl font-bold text-amber-400 mb-6">The Social Media Factory Trial</h2>
              <QuizEngine quiz={module.quiz} moduleId="module-12" />
            </div>
          ) : (
            <div>
              <div className="mb-6">
              ...
              <LessonNavigation moduleId="module-12" currentLessonIndex={currentLessonIndex} totalLessons={totalLessons} showQuiz={showQuiz} onNext={handleNext} onPrevious={handlePrevious} />
            </div>
          )}
        </main>
      </div>
