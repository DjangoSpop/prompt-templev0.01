 ...
 *
 * Build a persistent AI assistant with short-term and long-term memory
 */
...
export default function Module11Page() {
  const module = getModuleById('module-11');
  const [currentLessonIndex, setCurrentLessonIndex] = useState(0);
  const [showQuiz, setShowQuiz] = useState(false);
  const unlockedModules = useAcademyStore((state) => state.unlockedModules);
  const progress = useAcademyStore(selectModuleProgress('module-11'));

  const isUnlocked = unlockedModules.includes('module-11');
    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-obsidian-900">
      <header className="flex-shrink-0 bg-obsidian-800 border-b border-royal-gold-500/20">
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center gap-4 mb-3">
            <Link href="/academy" className="text-gray-300 hover:text-amber-400 transition-colors">
            ...
            <div>
              ...
              <p className="text-sm text-gray-300">
                {completedLessons} of {totalLessons} lessons • {Math.round(progressPercent)}% complete
              </p>
            </div>
          </div>
      ...
      <div className="flex flex-1 min-h-0">
        <ModuleSidebar
          ...
          showQuiz={showQuiz}
          onLessonClick={(index) => { setShowQuiz(false); setCurrentLessonIndex(index); }}
        ...
        <main className="flex-1 overflow-y-auto p-6 md:p-10">
          {showQuiz ? (
            <div>
              <h2 className="text-3xl font-bold text-amber-400 mb-6">The AI Memory Trial</h2>
              <QuizEngine quiz={module.quiz} moduleId="module-11" />
            </div>
          ) : (
            <div>
              <div className="mb-6">
              ...
              <LessonNavigation moduleId="module-11" currentLessonIndex={currentLessonIndex} totalLessons={totalLessons} showQuiz={showQuiz} onNext={handleNext} onPrevious={handlePrevious} />
            </div>
          )}
        </main>
      </div>
