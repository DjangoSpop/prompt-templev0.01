...
export function AppLayoutWithSidebar({ children }: AppLayoutWithSidebarProps) {
  ...
  return (
    <div className="flex min-h-screen min-h-dvh min-w-0 overflow-x-hidden bg-gradient-to-br from-background via-papyrus/5 to-background/80">
      ...
      <main
        className={cn(
          ...
          !isAnonymousLanding && 'md:ml-[72px]',
          !isAnonymousLanding && (isMounted ? 'lg:ml-[var(--sidebar-width)]' : 'lg:ml-[260px]')
        )}
        style={{
      ...
      >
        ...
        {/* Content wrapper */}
        {isAnonymousLanding ? (
          /* Landing page: full-width, no card wrapper */
          <div className="min-h-screen min-w-0">
            {children}
          </div>
        ) : (
          /* Authenticated pages: card wrapper with padding */
          <div className="min-h-screen min-w-0">
      ...
      {!isAnonymousLanding && <BottomNav />}
      {/* Floating Action Button — mobile only */}
      {!isAnonymousLanding && <FloatingActionButton />}
    </div>
  );
}
...
export function AppLayoutWithSidebarFull({ children }: AppLayoutWithSidebarProps) {
  ...
  return (
    <div className="flex min-h-screen min-h-dvh min-w-0 overflow-x-hidden bg-gradient-to-br from-background via-papyrus/5 to-background/80">
      ...
      <main
        className={cn(
          'flex-1 min-w-0 min-h-screen transition-all duration-300 ease-in-out',
          'md:ml-[72px]',
